...
export function DetailDrawer({ tx, onClose }: DetailDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, zIndex: 201, width: "440px", background: "var(--bg)", borderLeft: "1px solid var(--border)", display: "flex", flexDirection: "column", boxShadow: "-8px 0 40px rgba(0,0,0,0.12)" }}>
          ...
          <div style={{ padding: "16px 20px", borderRadius: "12px", background: scoreBg, border: `1px solid ${scoreBorder}` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: "11px", color: "var(--tm)" }}>{scoreLabel}</div>
              </div>
            </div>
          </div>
        ...
        {(tx.status === "pending" || tx.status === "high-alert") && <DrawerActions />}
      </div>
    </>
  );
}
...
function DrawerHeader({ tx, onClose }: { tx: ProcurementTransaction; onClose: () => void }) {
  return (
    <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <div>
        <div style={{ fontSize: "11px", color: "var(--em)", fontWeight: 600, letterSpacing: "0.8px", marginBottom: "2px" }}>
          {tx.id} · {tx.docNumber}
        </div>
