...
import { importProcurements } from "@/services/procurementService";
...
export function ImportProcurementModal({ isOpen, onClose, isMobile, onSuccess }: ImportModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setResult(null);
    try {
      const res: any = await importProcurements(file, dispatchMl);
  ...
  const handleDownloadSample = () => {
    const headers = "purchaseId,purchaseDate,vendorName,itemDescription,department,amountTotal,procurementMethod,employeeExternalRef\n";
    const sample1 = "PO-001,2026-05-15,PT Teknologi Terdepan,Pengadaan 10 unit laptop,IT,150000000,pengadaan_langsung,EMP-001\n";
    const sample2 = "PO-002,2026-05-16,CV Maju Bersama,Pembelian ATK Kantor,Finance,5000000,e_purchasing,EMP-002\n";
    ...
    link.href = url;
    link.setAttribute("download", "Template_Import_Pengadaan.csv");
    ...
    <>
      ...
      <div style={{ position: "fixed", zIndex: 201, display: "flex", flexDirection: "column", ...(isMobile ? { left: 0, right: 0, bottom: 0, background: "var(--bg)", borderRadius: "20px 20px 0 0", maxHeight: "90vh", animation: "slideInBottom 0.28s cubic-bezier(0.32,0.72,0,1)" } : { top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "100%", maxWidth: "480px", background: "var(--bg)", borderRadius: "20px", animation: "slideUp 0.24s cubic-bezier(0.22,1,0.36,1)" }), boxShadow: "0 32px 80px rgba(0,0,0,0.22)", overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h3 style={{ fontFamily: "'Syne', sans-serif", fontSize: "18px", fontWeight: 700, color: "var(--tp)", margin: 0 }}>Import Data Pengadaan</h3>
        ...
        <div style={{ padding: "24px", overflowY: "auto", maxHeight: isMobile ? "60vh" : "420px" }}>
          {!result ? (
            <form id="import-procurement-form" onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              <div
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div style={{ padding: "16px", borderRadius: "12px", background: allSuccess ? "rgba(16,185,129,0.08)" : allFailed ? "rgba(239,68,68,0.08)" : "rgba(245,158,11,0.08)", border: `1px solid ${allSuccess ? "rgba(16,185,129,0.25)" : allFailed ? "rgba(239,68,68,0.25)" : "rgba(245,158,11,0.25)"}`, display: "flex", alignItems: "center", gap: "12px" }}>
                {allSuccess && <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>}
                {allFailed && <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>}
                {partialSuccess && <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>}
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: "var(--tm)", marginTop: "2px" }}>
                    Total {result.totalRows} baris — <span style={{ color: "#10b981", fontWeight: 600 }}>{result.successRows} berhasil</span>
                    {result.failedRows > 0 && <>, <span style={{ color: "#ef4444", fontWeight: 600 }}>{result.failedRows} gagal</span></>}
                  </div>
                </div>
              </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px", maxHeight: "200px", overflowY: "auto" }}>
                    {result.errors.map((err, i) => (
                      <div key={i} style={{ padding: "10px 12px", borderRadius: "8px", background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", display: "flex", gap: "10px", alignItems: "flex-start" }}>
                        ...
                        <div style={{ flex: 1 }}>
                          {err.purchaseId && <div style={{ fontSize: "11px", color: "var(--tm)", marginBottom: "2px" }}>ID: {err.purchaseId}</div>}
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", background: "var(--surface-2)", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
          ...
          {!result && (
            <button type="submit" form="import-procurement-form" disabled={loading} style={{ padding: "10px 20px", borderRadius: "10px", border: "none", background: loading ? "var(--surface-2)" : "linear-gradient(135deg, var(--em), var(--em2))", color: loading ? "var(--tm)" : "#fff", fontSize: "13px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'DM Sans', sans-serif", boxShadow: loading ? "none" : "0 4px 16px rgba(16,185,129,0.25)" }}>
