...
export default function ProcurementSummaryCards({ data, isMobile, isTablet }: ProcurementSummaryCardsProps) {
  ...
  const highAlertCount  = data.filter(t => t.status === "high-alert").length;
  const pendingCount    = data.filter(t => t.status === "pending").length;
  const approvedCount   = data.filter(t => t.status === "approved" || t.status === "auto-approved").length;
  // const rejectedCount   = data.filter(t => t.status === "rejected").length; // Bisa dihapus kalau nggak dipakai di trend lagi
      ...
      {cards.map((card) => {
        ...
        return (
          ...
          >
            ...
            {/* Value + sub (Trend section deleted) */}
            <div>
              <div style={{
