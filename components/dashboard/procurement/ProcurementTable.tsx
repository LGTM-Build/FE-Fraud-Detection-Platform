...
import {
  ...
  procurementMethodLabels, type ProcurementTransaction,
        ...
        return (
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <span style={{ fontSize: "12px", fontWeight: 700, color: "var(--em)" }}>{t.purchaseId ?? t.id.slice(0, 8)}</span>
                <span style={{ fontSize: "10px", color: "var(--tm)", marginLeft: "6px" }}>{fmtDate(t.purchaseDate)}</span>
              </div>
            ...
            <div>
              <div style={{ fontSize: "13px", fontWeight: 500, color: "var(--tp)" }}>{t.vendorName}</div>
              <div style={{ fontSize: "11px", color: "var(--tm)" }}>{t.itemDescription}</div>
            </div>
            ...
            return (
              ...
              >
                <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                  <div style={{ fontSize: "12px", fontWeight: 600, color: "var(--em)" }}>{t.purchaseId ?? "—"}</div>
                  <div style={{ fontSize: "10px", color: "var(--tm)", fontFamily: "monospace" }}>{t.id.slice(0, 8)}…</div>
                </td>
                <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                  <span style={{ fontSize: "12px", color: "var(--tm)" }}>{fmtDate(t.purchaseDate)}</span>
                </td>
                <td style={{ padding: "13px 16px", maxWidth: "180px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 500, color: "var(--tp)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.vendorName}</div>
                </td>
                <td style={{ padding: "13px 16px", maxWidth: "200px" }}>
                  ...
                  <div style={{ fontSize: "11px", color: "var(--tm)", marginTop: "2px" }}>{t.employeeName ?? "—"} · {t.requesterName}</div>
                </td>
                <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                ...
                <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
                  <span style={{ padding: "3px 10px", borderRadius: "100px", fontSize: "11px", background: "var(--em-subtle)", border: "1px solid var(--border)", color: "var(--ts)" }}>
                    {procurementMethodLabels[t.procurementMethod] ?? t.procurementMethod}
                  </span>
                </td>
                <td style={{ padding: "13px 16px", whiteSpace: "nowrap" }}>
...
export default function ProcurementTable({ data, onSelect, isMobile }: TableProps) {
  ...
  const filtered = q
    ? data.filter(t =>
        (t.purchaseId ?? "").toLowerCase().includes(q) ||
