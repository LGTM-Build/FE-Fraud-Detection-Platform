...
export function ReportConfig({
  scope, periodStart, periodEnd, format, includeCharts, includeRaw,
  onScopeChange, onPeriodStartChange, onPeriodEndChange,
  onFormatChange, onIncludeChartsChange, onIncludeRawChange,
  isMobile,
}: ReportConfigProps) {
  return (
    <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-b)", borderRadius: "16px", padding: isMobile ? "16px" : "20px 24px", display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: "4px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "10px", padding: "3px" }}>
          {SCOPES.map(s => (
            <button key={s} onClick={() => onScopeChange(s)} style={{
              flex: 1, padding: isMobile ? "6px 8px" : "7px 12px",
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          {[
            ...
            <div key={f.label}>
              ...
              <input type="date" value={f.value} onChange={e => f.onChange(e.target.value)} style={{
                width: "100%", padding: isMobile ? "8px 10px" : "9px 12px",
                borderRadius: "10px", border: "1px solid var(--border)",
                background: "var(--surface-2)", color: "var(--tp)",
        ...
        <div style={{ display: "flex", gap: "6px", marginTop: "10px", flexWrap: "wrap" }}>
          {PERIOD_SHORTCUTS.map(p => (
            <button key={p.label} onClick={() => { onPeriodStartChange(p.start); onPeriodEndChange(p.end); }} style={{
              padding: "4px 12px", borderRadius: "100px",
              border: "1px solid var(--border)", background: "var(--surface-2)",
              color: "var(--ts)", fontSize: "11px", cursor: "pointer",
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          {FORMATS.map(f => (
            <div key={f.key} onClick={() => onFormatChange(f.key)} style={{
              ...
              <span style={{ fontSize: isMobile ? "16px" : "20px" }}>{f.icon}</span>
              <div>
                <div style={{ fontSize: isMobile ? "12px" : "13px", fontWeight: 500, color: format === f.key ? "var(--em)" : "var(--tp)" }}>{f.label}</div>
                {!isMobile && <div style={{ fontSize: "11px", color: "var(--tm)" }}>{f.desc}</div>}
              </div>
              {format === f.key && (
          ...
          {[
            ...
            <div key={opt.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: isMobile ? "12px" : "13px", color: "var(--tp)", fontWeight: 400 }}>{opt.label}</div>
                {!isMobile && <div style={{ fontSize: "11px", color: "var(--tm)" }}>{opt.desc}</div>}
              </div>
              <div onClick={() => opt.onChange(!opt.value)} style={{
