...
export function ReportTypeSelector({ value, onChange, isMobile }: ReportTypeSelectorProps) {
  return (
    <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-b)", borderRadius: "16px", padding: isMobile ? "16px" : "20px 24px" }}>
      ...
      <div style={{
        ...
        {REPORT_TYPES.map(t => (
          <div key={t} onClick={() => onChange(t)} style={{
            ...
            <div style={{ fontSize: isMobile ? "12px" : "13px", fontWeight: 500, color: value === t ? "var(--em)" : "var(--tp)", marginBottom: "4px" }}>
              {typeLabels[t]}
            </div>
