    ...
    <div style={{
      ...
      {items.map(s => (
        <div key={s.label} style={{
          ...
          <span style={{ fontSize: isMobile ? "11px" : "13px", color: s.color, fontWeight: 500 }}>{s.label}</span>
          <span style={{ fontFamily: "'Syne', sans-serif", fontSize: isMobile ? "22px" : "28px", fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.count}</span>
        </div>
      ))}
    </div>
  );
